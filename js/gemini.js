window.Gemini = (function(){
  ...
  async function uploadSource(_base, file, onProgress){
    ...
    const response = await fetch(SUPABASE_FUNCTION("upload-to-gemini"), { method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ ...auth, storage_path:path, file_name:file.name, mime_type:file.type || "application/octet-stream", size_bytes:file.size }) });
  ...
  async function pollUntilActive(_base, fileApiName, onState){
    ...
    for(let i=0;i<20;i++){
      const res = await fetch(`${SUPABASE_FUNCTION("file-status")}?name=${encodeURIComponent(fileApiName)}&code=${encodeURIComponent(auth.code)}&device_id=${encodeURIComponent(auth.device_id)}`);
  ...
  async function streamGenerate(_base, systemInstruction, contents, onChunk){
    const auth = activation();
    const res = await fetch(SUPABASE_FUNCTION("ai-chat"), { method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ ...auth, systemInstruction, contents }) });
